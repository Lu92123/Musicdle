...
> una GPU NVIDIA, si no usa CPU), el resto de dependencias y **ffmpeg** vía winget. Además te
> pide **CLIENT_ID y CLIENT_SECRET** con instrucciones paso a paso y los guarda en `.env`.
> Para cambiarlas más tarde: `setup.bat --creds-only`. Después abrí `Iniciar Musicdle.bat`
> para jugar. Los pasos de abajo son la versión manual.
